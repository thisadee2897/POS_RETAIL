...
const MemberTypeSetting = () => {
    ...
    const columnData = [
        ...
        {
            name: 'สถานะการใช้งาน',
            selector: row => <><Switchstatus value={row.member_type_active} /></>,
            sortable: true,
        },
        {
            name: 'แก้ไข',
    ...
    const onClickSave = (data) => {
        if (data.master_member_type_id) {
            axios.post(UrlApi() + 'update_membertype_data', data)
                .then(res => {
                    if (res.data) {
            ...
            data.member_type_active = data.member_type_active != undefined ? data.member_type_active : true
            axios.post(UrlApi() + 'add_membertype_data', data)
                .then(res => {
                    if (res.data) {
                        setAlertMessages("เพิ่มข้อมูลสำเร็จ")
                        setAlertSuccess(true)
    ...
    const columnDialog = [
        { name: 'ชือ', type: "input_text", key: "master_member_type_name", validate: true },
        { name: 'สถานะการใช้งาน', type: "switch_status", key: "member_type_active" }
    ]

    const getDialog = () => {
        return (
            <DialogMaster
                keys="master_member_type_id"
